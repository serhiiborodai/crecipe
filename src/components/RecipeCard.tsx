...
export default function RecipeCard({ recipe }: RecipeCardProps) {
  ...
  return (
    <Link href={`/recipes/${recipe.id}`}>
      <article className="group relative bg-zinc-900/50 rounded-2xl overflow-hidden border border-zinc-800/50 hover:border-amber-500/30 transition-all duration-500 hover:shadow-2xl hover:shadow-amber-500/5">
        {/* Обложка */}
        <div className="relative h-56 bg-gradient-to-br from-zinc-800 to-zinc-900 overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center text-6xl opacity-30 group-hover:scale-110 transition-transform duration-700">
            ...
            {recipe.category === 'Супы' && '🍜'}
            {recipe.category === 'Салаты' && '🥗'}
            {recipe.category === 'Морепродукты' && '🦐'}
            {!recipe.category && '🍽️'}
          </div>
          ...
          <div className="absolute top-4 right-4">
            {isPurchased ? (
              <span className="px-3 py-1.5 bg-emerald-500 text-white text-sm font-medium rounded-full flex items-center gap-1.5 shadow-lg">
              ...
              <span className="px-3 py-1.5 bg-amber-500 text-zinc-900 text-sm font-bold rounded-full shadow-lg">
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-3 text-sm">
            {difficulty && (
              <span className={`px-2 py-1 rounded-md ${difficulty.color}`}>
                {difficulty.text}
              </span>
            )}
